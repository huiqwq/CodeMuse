...
> 当前实施版本：v0.8.0 完整文件操作与 Git 审查。具体变化见 [releases/v0.8.0.md](releases/v0.8.0.md)。
...
v0.8.0 可以分析、局部修改、创建/重命名/删除普通文本、受控验证、自动修复、只读 Git 审查，并在工作区未变化时恢复历史任务摘要。所有写入和执行仍需用户确认。它不能运行任意 Shell、自动安装依赖、执行 Git 写操作，也不能恢复崩溃时尚未结束的任务或跨工作区恢复。Git 归属基线和 `/undo` 只保存在当前进程。
...
```powershell
git tag -a v0.8.0 -m "CodeMuse v0.8.0"
