...
- 38 项自动测试及 TypeScript 静态检查。

当前版本仍不能创建、删除或重命名文件，不能执行任意 Shell 或 Git 写操作，也不会自动安装依赖。
...
| v0.6.0 | 自动错误修复 | 错误分类、位置提取、失败指纹、复测和停止策略 | 已完成，待提交 |
| v0.7.0 | 会话历史与恢复 | 本地会话存储、检查点、`/history`、`/resume` | 待开发 |
...
- 在工作区 `.codemuse/` 保存结构化会话。
- 记录任务、计划、工具调用、Diff、授权和验证结果。
- 增加 `/history`、`/resume` 和检查点。
- 会话恢复前校验工作区内容是否变化。
- 不保存明文 API Key。
...
- [版本变更记录](CHANGELOG.md)
- [v0.6.0 版本说明](docs/releases/v0.6.0.md)
