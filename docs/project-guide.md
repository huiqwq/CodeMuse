...
> 当前实施版本：v0.10.0 安全凭据与代码审查。具体变化见 [releases/v0.10.0.md](releases/v0.10.0.md)。
...
glm 和 glm-flash 共用 ZHIPUAI_API_KEY。关闭并重新打开 PowerShell 后，CodeMuse 会自动读取已保存凭据。macOS Keychain 和 Linux Secret Service 尚未实现，非 Windows 平台继续使用环境变量。
...
### 8.6 v0.11.0 安装、诊断与跨平台

- npm 全局发布、首次向导、/doctor 和可控日志。
- macOS Keychain、Linux Secret Service 和跨平台进程处理。

### 8.7 v0.12.0 稳定性与安全验收

- 端到端示例仓库、提示注入、危险脚本和恶意路径测试。
- 多模型 Tool Calling、取消、超时、并发变化和大项目性能。

### 8.8 v1.0.0 稳定开发闭环

正式完成需求、计划、上下文、修改、测试、修复、Git 审查和会话恢复，通过跨平台端到端验收后发布。
...
| v0.11.0 | 安装、诊断与跨平台 | npm 发布、向导、doctor、Keychain/Secret Service |
| v0.12.0 | 稳定性与安全 | 端到端、注入、兼容性和性能验收 |
...
```powershell
git tag -a v0.10.0 -m "CodeMuse v0.10.0"
