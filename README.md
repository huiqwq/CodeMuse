...
- 71 项自动测试及 TypeScript 静态检查。

当前版本不能执行任意 Shell 或 Git 写操作，不会自动安装依赖、commit 或 push。Windows 可将 API Key 保存为当前用户 DPAPI 密文；非 Windows 平台仍使用环境变量。Token 用量依赖模型服务返回 usage 字段；会话只在任务结束时保存，不支持崩溃后的中途恢复。
...
| v0.11.0 | 安装、诊断与跨平台 | npm 发布、向导、doctor、Keychain/Secret Service | 待开发 |
| v0.12.0 | 稳定性与安全验收 | 端到端、注入、兼容性和性能 | 待开发 |
| v1.0.0 | 稳定开发闭环 | 正式安装、分析、修改、验证、恢复 | 待开发 |
...
- npm 全局发布和首次启动向导。
- `/doctor`、可控日志和故障排查。
- macOS Keychain、Linux Secret Service 和跨平台进程兼容。
