...
> 当前实施版本：v0.7.0 本地会话历史与恢复。具体变化见 [releases/v0.7.0.md](releases/v0.7.0.md)。
...
v0.7.0 可以分析、局部修改、受控验证、自动修复，并在工作区未变化时恢复历史任务摘要。所有写入和执行仍需用户确认。它不能创建、删除或重命名文件，不能运行任意 Shell、安装依赖、执行 Git 写操作，也不能恢复崩溃时尚未结束的任务或跨工作区恢复。
...
```powershell
git tag -a v0.7.0 -m "CodeMuse v0.7.0"
