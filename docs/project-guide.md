...
> 当前实施版本：v0.6.0 错误诊断与受控自动修复。具体变化见 [releases/v0.6.0.md](releases/v0.6.0.md)。
...
v0.6.0 可以分析和局部修改代码，运行允许的 npm 验证脚本，并在真实模型模式下根据失败证据继续修复和复测。所有写入和执行仍需用户确认。它不能创建、删除或重命名文件，不能运行任意 Shell、安装依赖、启动开发服务器、执行 Git 写操作、恢复历史会话或处理 Monorepo 子包。
...
“OpenAI-compatible”是协议兼容层，不代表只能使用 OpenAI。DeepSeek、GLM 和其他兼容服务可以共用 Provider 适配器。连接测试、模型切换、重试和 Token 统计安排在 v0.9.0。

真实 API Key 不得写入代码、`.env`、README、Issue、截图或 Git 提交。

## 8. 已完成闭环与后续模块

### 8.1 v0.1.0 至 v0.6.0 已完成
...
- 在 `.codemuse/` 保存任务、计划、上下文摘要、工具调用、Diff、授权和验证结果。
- 增加检查点、`/history` 和 `/resume`。
- 恢复前校验工作区变化。
- 不保存明文 API Key。

### 8.3 v0.8.0 完整文件操作与 Git 审查

- 安全创建、重命名和删除文本文件。
- 所有操作展示 Diff 或文件清单并请求确认。
- 只读 Git Status、Diff 和分支信息。
- 区分用户原有修改与 Agent 修改。
- 默认不自动 commit 或 push。

### 8.4 v0.9.0 多模型与 API 管理

- 完善 DeepSeek、GLM、OpenAI 和自定义兼容 Provider。
- 本机 Provider 配置、连接测试和会话内模型切换。
- 网络超时、429、服务端错误、重试和 Token 统计。
- API Key 不写入项目或会话。
...
```powershell
git tag -a v0.6.0 -m "CodeMuse v0.6.0"
