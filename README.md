...
- 51 项自动测试及 TypeScript 静态检查。

当前版本不能执行任意 Shell 或 Git 写操作，不会自动安装依赖、commit 或 push。新建文件的父目录必须已经存在，文件生命周期工具仅支持普通 UTF-8 文本；会话只在任务结束时保存，不支持崩溃后的中途恢复。
...
- [版本变更记录](CHANGELOG.md)
- [v0.8.0 版本说明](docs/releases/v0.8.0.md)
